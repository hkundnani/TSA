...
result = pd.merge(moods_sent_mean, moods_poms_mean, how = 'outer', on=['Date'])
...
pl.suptitle('Moods vs Compound value for Cisco tweets')
pl.title('POMS')
...
ax1.plot(result['Date'], result['Compound'], 'y', label='Compound')
...
ax2.plot(result['Date'], result['Compound'], 'y', label='Compound')
